...
let _ = assoc (-1, "adam",[("adam",6);("moose", 9);("adam", 3)]);;
...
let _ = removeDuplicates [1;2;2;2;3;2;2;2;4];;
...
let rec exprToString e = match e with
  ...
  | SumOfSquares (e1, e2, e3) -> 
      "sumsqr(" ^ exprToString e1 ^ "," ^ exprToString e2 ^ "," ^ 
      exprToString e3 ^ ")"
;;

let sampleExpr2 = Thresh(VarX,VarY,VarX,Average(VarX,VarY));;
...
let rec eval (e,x,y) = match e with
  ...
  | SumOfSquares (e1,e2,e3) ->
      (eval(e1,x,y) ** 2) + (eval(e2,x,y) ** 2) + (eval(e3,x,y) ** 2)

;;

let _ = eval (Times(VarX,VarY), 50.0, 100.0);;
