    ...
    match rest with 
      ...
      | h::t -> 
          let seen' = if(List.mem h seen = false) then h::seen 
            else seen in  (*XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX*)
          let rest' = t in 
            helper (seen',rest') 
  in
    List.rev (helper ([],l));;
...
let rec exprToString e = match e with 
  ...
  | SumPercent(e1, e2, e3) -> "((" ^ exprToString e1 ^ "+" ^
...
let buildSumPercent(e)	           = SumPercent(e)
...
let rec eval (e,x,y) = match e with
  ...
  | SumPercent(e1, e2, e3) -> (eval (e1, x, y) + eval(e2, x, y) 
                               + eval(e3, x, y)) *. 0.01
...
let _ = doRandomColor(8, 312, 91)
let _ = doRandomColor(10, 151, 130)
let _ = doRandomColor(8, 2212, 7437)

let _ = doRandomGray(8, 72, 2140)
let _ = doRandomGray(10, 66, 6)
let _ = doRandomGray(8, 2212, 7437)
