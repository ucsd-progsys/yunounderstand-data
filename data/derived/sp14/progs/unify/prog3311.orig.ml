...
let rec addPersHelper n ret =
  let l = digitsOfInt n in
  let x = sumList l in
    if x = n then ret else addPersHelper x ret+1

let additivePersistence n = 
  let l = digitsOfInt n in
  let x = sumList l in
    if x = n then 0 else addPersHelper x 1
...
let rec palinHelper normal reverse =
  match normal with
    | [] -> true
    | x::xs -> if x = List.hd reverse then palinHelper xs List.tl reverse else false
