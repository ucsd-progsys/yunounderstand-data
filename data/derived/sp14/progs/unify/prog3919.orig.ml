...
let rec eval (e,x,y) = match e with
  ...
  | VarY ->  y
  | VarX ->  x
  | Abs (a) -> if eval (a,x,y) a < 0 then eval (a,x,y) *. (-1.0) else eval (a,x,y)
;;

(*XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX*)
let _ = eval (Average(VarX,VarY), 1.0, 3.0);;
let _ = eval (Times(VarX,VarY), 3.0, 2.5);;
...
let sampleExpr =
  buildCosine(buildSine(buildTimes(buildCosine(buildAverage(buildCosine(buildX()),buildTimes(buildCosine (buildCosine (buildAverage(buildTimes (buildY(),buildY()),buildCosine (buildX())))),
...
let _ = doRandomGray (8,31,22);;
let _ = doRandomGray (8,31,23);;
let _ = doRandomGray (8,31,41);;
let _ = doRandomGray (8,31,12);;
...
let _ = doRandomColor (5,32,37);;

let _ = doRandomColor (5,32,22);;

let _ = doRandomColor (5,32,54);;

let _ = doRandomColor (5,32,33);;
