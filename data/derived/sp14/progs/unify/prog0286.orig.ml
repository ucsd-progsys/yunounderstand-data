...
let rec sumList xs = (*XXXXXXXXXXXXXXXXXXXXXX*)
  match xs with
    | [] -> 0
    | x::xs -> x + sumList xs;;
...
let rec digitsOfInt n = (*XXXXXXXXXXXXXXXXXXXXXXXXXX*)
  if n < 0 then []
  else
    let rec loop n dig_list = 
      if n = 0 then dig_list
      else loop (n/10) (n mod 10::dig_list) in
      match n with
        | _ -> loop n [];;
...
(*XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX*)
let additivePersistence n = (*XXXXXXXXXXXXXX*)
  let count = 0 in
  let rec add ret count = 
    if ret < 10 then count
    else
      let count = count + 1 in
        add(sumList(digitsOfInt ret) ) count in match n with
    | 0 -> 0
    | _ -> add n count;;
(*XXXXXXXXXXXXXXX
XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
...
(*XXXXXXXXXXXXXXXXXXXXXXXXX*)
let digitalRoot n = (*XXXXXXXXXXXXXX*)

  let rec loop x = 
    if x < 10 then x
    else loop (sumList (digitsOfInt (abs x) ) ) in match n with
    |0 -> 0
    |_ -> loop n;;
...
let listReverse l = (*XXXXXXXXXXXXXX*)
...
let palindrome w = (*XXXXXXXXXXXXXX*)
