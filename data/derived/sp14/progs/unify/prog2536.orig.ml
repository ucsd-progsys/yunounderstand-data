...
let whilesFun f' b' = let fOfB = f' (b') in (b' ,fOfB = b');;
...
let rec foo t = match t with
  | Leaf n -> 1
  | Node (t1, t2) -> foo t1 + foo t2
in foo (Node(Node(Leaf 1, Leaf 2),Leaf 3)) ;;
(*XXXXXXXXXXXXXXXX*)
(*XXXXXXXXXXXXXXXX*)
(*XXXXXXXXXXXXXXXX*)
(*XXXXXXXXXXXXXXXX*)
