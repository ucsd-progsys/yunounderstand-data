  ...
  if depth <= 0 then
    ...
    let first_forced = build(rand, depth - 1) in
      match exp_rand with
        ...
        | _ -> buildThresh(first_forced, build(rand, depth - 1), 
...
let buildTimes(e1,e2)              = Times(e1,e2)
let buildThresh(a,b,a_less,b_less) = Thresh(a,b,a_less,b_less)
let buildAcossin(e1,e2)		   = Accosin(e1,e2)
...
let _ = removeDuplicates long_list;;
...
doRandomGray(8, 1000, 1005);;
doRandomColor(8, 1000, 1500);;
