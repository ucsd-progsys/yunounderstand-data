...
let buildFiboPlus(e1,e2,e3,e4,e5)  = FiboPlus(e1,e2,e3)
...
let rec eval (e,x,y) = match e with VarX -> x
                                  ...
                                  | FiboPlus (ex1,ex2,ex3,ex4,ex5) -> ((eval (ex1,x,y))*.(eval (ex1,x,y)+.eval (ex2,x,y))*.(eval (ex1,x,y)+.eval (ex2,x,y)+.eval (ex3,x,y)))
