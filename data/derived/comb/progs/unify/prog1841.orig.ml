...
let fixpoint (f,b) = wwhile ( 
                       let f' b' =
                         let bb =
                           f b in ( bb, bb = f b ) in wwhile( f', b) , b )
;;
...
let _ = eval (Sum3(Factorial(VarX), Factorial(VarY), Factorial(VarY)), 5.0, 4.0 );;
  ...
  if depth <= 0 then
    ...
    let x = rand( 0, 6 ) in 
      match x with
        ...
        | 7
          -> buildFactorial( build( rand, depth -1 ) )
        | _
          -> buildSum3( build( rand, depth -1 ), build( rand, depth -1 ), build( rand, depth -1 ) )
;;
...
let g1 () = ( 76, 9239, 32432   ) ;;
let g2 () = ( 1753, 13, 999     ) ;;
let g3 () = ( 555, 30712, 11406 ) ;;

let c1 () = ( 76, 9239, 32432         ) ;;
let c2 () = ( 200000000, 30332, 10356 ) ;; 
let c3 () = ( 77777, 40613, 410       ) ;;
