...
let rec eval (e,x,y) = match e with
  ...
  | VarY ->  y
  | VarX ->  x
  | Abs (a) -> if eval (a,x,y) < 0.0 then eval (a,x,y) *. (-1.0) else eval (a,x,y)
  | Weird (a,b,c) -> if eval (a,x,y) > 0.0 then eval (b,x,y) else eval (c,x,y)
;;
...
let _ = doRandomColor (8,73,37);;
let _ = doRandomColor (8,73,22);;
let _ = doRandomColor (8,73,54);;
let _ = doRandomColor (8,73,33);;
