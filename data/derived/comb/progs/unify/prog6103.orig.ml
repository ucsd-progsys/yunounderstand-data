...
let bigMul l1 l2 = 
  let f a x =  match a with 
    |(r,v) -> let sum = intListToInt (mulByDigit (intListToInt l1) [x]) in 
          if (sum + r) > 9
          then (intListToInt (carryFunc (mulByDigit (intListToInt l1) [x])),((sum+r) mod 10)::v) 
          else  (0, ((sum+r) mod 10)::v)
  in
  let base = (0,[]) in
  let args = l2  in
  let (x, res) = List.fold_left f base args 
  in List.rev( [(x/10000) mod 10]@[(x/1000) mod 10]@[(x/100) mod 10]@[(x/10) mod 10]@[(x/1) x mod 10])@res 
