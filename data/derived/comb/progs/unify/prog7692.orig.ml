...
let rec digitsOfIntHelper n = if (n < 1) then 
    [] else
    digitsOfIntHelper ((n-(n mod 10))/10) @ [n mod 10]
...
let rec aPHelper n  = if (n >= 10) then 
    sumList (digitsOfInt n) else
    n

let rec digitalRoot n = if (aPHelper n) >= 10 then 
    additivePersistence (aPHelper n)
  else aPHelper n
