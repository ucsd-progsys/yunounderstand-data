...
let rec assoc (d,k,l) = 
  match l with
    | [] -> d
    | h::t -> let(word,value) = h in 
          if(k = word) then value
          else assoc(d,k,t)
...
*)
let rec wwhile (f,b) = 
  let (res,check) = f b in
    if(check) then wwhile(f, res)
    else res
...
let fixpoint (f,b) = wwhile (let test x = let meh = f x in  (meh, meh != x) in test,b)
