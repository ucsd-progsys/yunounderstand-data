...
let rec eval (e,x,y) = 
  match e with 
    ...
    | Power(a, b) -> if(x < 1.0 && x > -1.0 || y < 1.0 && y > -1.0) then 0.5 else eval(a, x, y) ** eval(b, x, y)
...
let rec build (rand, depth) = 
  let m = 7 in 
    match (rand(243, 98723), depth) with 
      ...
      | (x, n) when n > 0 && x mod m = 0 -> buildSine(build(rand, n - 1))
      | (x, n) when n > 0 && x mod m = 1 -> buildCosine(build(rand, n - 1))
      | (x, n) when n > 0 && x mod m = 2 -> buildAverage(build(rand, n - 1), build(rand, n - 1))
      | (x, n) when n > 0 && x mod m = 3 -> buildTimes(build(rand, n - 1), build(rand, n - 1))
      | (x, n) when n > 0 && x mod m = 4 -> buildPower(build(rand, n - 1), build(rand, n - 1))
      | (x, n) when n > 0 && x mod m = 5 -> buildTower(build(rand, n - 1), build(rand, n - 1), build(rand, n - 1))
