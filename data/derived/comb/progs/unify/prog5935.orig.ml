...
let rec exprToString e = 
  match e with
    ...
    | Boring (e1,e2) -> "3*"^exprToString e1^"+4*"^exprToString e2
    | SumTan (e1,e2,e3) -> "tan("^exprToString e1^")+tan("^exprToString e2^
                           ")+tan("^exprToString e3^")"
...
let buildBoring(e1,e2)	           = Boring(e1,e2)
let buildSumTan(e1,e2,e3)          = SumTan(e1,e2,e3)
...
let rec eval (e,x,y) = 
  match e with
    ...
    | SumTan(e1,e2,e3) -> tan(eval(e1,x,y))+.tan(eval(e2,x,y))+.
                          tan(eval(e3,x,y))
...
let rec build (rand, depth) = match depth with
  ...
  | _ -> let x = rand (0,8) in match x with
        | 0 -> buildX()
        | 1 -> buildY()
        ...
        | 7 -> buildBoring(build (rand, depth -1),build (rand, depth -1))
        | 8 -> buildSumTan(build (rand, depth -1),build (rand, depth -1),
                           build (rand, depth -1))
        | _ -> buildX()
