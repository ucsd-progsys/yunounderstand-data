...
let rec exprToString e = match e with 
  ...
  |Smallest (e1, e2, e3) -> "(" ^ exprToString e1 ^ " < " ^ exprToString e2 ^ " ? ("^ exprToString e1 ^ " < " ^ exprToString e2 ^ " ? " ^ exprToString e1 ^ " : " ^ exprToString e2 ^ ") : (" ^ exprToString e2 ^ " < " ^ exprToString e3 ^ " ? " ^ exprToString e2 ^ " : " ^ exprToString e3 ^ "))"
...
let buildSmallest(e1, e2, e3)      = Smallest(e1, e2, e3)
...
let rec eval (e,x,y) = match e with
  ...
  |Smallest (e1, e2, e3) -> if eval(e1, x, y) < eval(e2, x, y) then (if eval(e1, x, y) < eval(e3, x, y) then eval(e1, x, y) else eval(e3, x, y)) 
      else (if eval(e2, x, y) < eval(e3, x, y) then eval(e2, x, y) else eval(e3, x, y))
