...
let rec exprToString e = 
  let ex = exprToString in 
    match e with
      ...
      | Power(x,y) -> ex(x)^"^"^ex(y)
      | TowerInv(x,y, z) -> "(1/("^ex(x)^"^"^ex(y)^"^"^ex(z)^"))"  

(*XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX*)

let sampleExpr1 = Thresh(VarX,VarY,VarX,(Times(Sine(VarX),Cosine(Average(VarX,VarY)))));;
let sampleExpr5 = TowerInv(VarX, VarY, VarY)
...
let buildTowerInv(a, b, c)         = TowerInv(a, b, c)
...
let rec eval (e,x,y) = 
  match e with 
    ...
    | TowerInv(a, b, c) -> if(x < 1.0 && x > -1.0 || y < 1.0 && y > -1.0) then 0.5 else 1 /. (eval(a, x, y) ** eval(b, x, y) ** eval(c, x, y))
