...
let padZero l1 l2 = let len = List.length l1 - List.length l2 in
    if len <= 0 then ((clone 0 (-1*len))@l1,l2)
...
let bigAdd l1 l2 = 
  let add (l1, l2) = 
    let f a x = let (x1,x2) = x in let (o, r) = a in 
        (((x1+x2+o)/10),((x1+x2+o) mod 10)::r) in
    let base = (0,[]) in
    let args = List.rev (List.combine (clone 0 1 :: l1) (clone 0 1 :: l2)) in
