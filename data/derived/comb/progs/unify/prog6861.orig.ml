...
let padZero l1 l2 = (clone 0 (List.length l2 - List.length l1) @ l1, clone 0 (List.length l1 - List.length l2) @ l2)
...
let bigAdd l1 l2 = 
  let add (l1, l2) = 
    let f a x = 
      let (num1, num2) = x in 
      let (carry, sum) = a in
        (if num1 + num2 + carry >= 10 then 1 else 0, (num1 + num2 + carry) mod 10::sum) in
    let base = (0, []) in
