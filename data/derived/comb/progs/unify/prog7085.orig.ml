...
let rec clone x n = 
  if n <= 0 then []
  else List.append [x] (clone x (n-1))
...
let padZero l1 l2 = 
  ...
  let y = List.length l2 in
    if x > y then (l1, List.append (clone 0 (x-y)) l2)
    else if x < y then (List.append (clone 0 (y-x)) l1, l2)
    else (l1, l2)
...
let bigAdd l1 l2 = 
  let add (l1, l2) = 
    let f a x = 
      let (lh1, lh2) = x in
      let (carry, res) = a in
      let num = lh1 + lh2 + carry in
        ((num / 10), (num mod 10)::res) in
    let base = (0,[]) in
...
let bigMul l1 l2 = 
  let f a x = 
    let (place, res) = a in
      if place <= 0 then (place+1, res + (mulByDigit x res))
