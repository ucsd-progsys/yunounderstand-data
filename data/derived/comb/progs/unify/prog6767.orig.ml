...
let rec exprToString e = match e with
  ...
  | Average (x3,x4)      -> 
      "((" ^ exprToString x3 ^ "+" ^ exprToString x4 ^ ")/2)"
  | Times (x5,x6)        -> exprToString x5 ^ "*" ^ exprToString x6
  | Thresh (x7,x8,x9,x0) -> 
      "(" ^ exprToString x7 ^ "<" ^ exprToString x8 ^ 
      "?" ^ exprToString x9 ^ ":" ^ exprToString x0 ^ ")"
...
let rec build (rand, depth) = 
  let r = if depth > 0 then rand(2,11) else rand(0,2) in
    match r with
      ...
      | 9 -> buildTimes(build(rand,depth-1),build(rand,depth-1))
