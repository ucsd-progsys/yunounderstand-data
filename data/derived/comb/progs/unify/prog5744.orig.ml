...
let rec exprToString e = match e with
  ...
  | Volume (e1, e2, e3) -> "(" ^ exprToString (e1) "*" ^ exprToString (e2) "*" ^ exprToString (e3) ^ ")"
...
let buildVolume(e1, e2, e3)		   = Volume(e1, e2, e3)
let buildTangent(e)		   = Tan(e)
...
let rec eval (e,x,y) = match e with
  ...
  | Volume (e1, e2, e3) -> (eval (e1, x, y,)) *. (eval (e2, x, y)) *. (eval (e3, x, y))
  | Tan (e1) -> tan (eval (e1, x, y))
...
let rec build (rand, depth) = 
  ...
  let alternate = rand(0, pumping) in 
    if depth = 0 then (
      if alternate mod 2 == 0 then buildX()
      else buildY()
    )
    else
      begin
        ...
        let result = match x with
          ...
          | 7 -> buildVolume(build(rand, (depth-1)), build(rand, (depth-1)), build(rand, (depth-1)))
          | 7 -> buildTangent(build(rand, (depth-1)))
        in result
      end
