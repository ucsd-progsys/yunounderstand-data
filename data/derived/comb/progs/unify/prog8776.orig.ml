...
let loseTail z = match listReverse z with
  |[] -> []
  |h::t -> t 

let getHeads y = match listReverse y with 
  | [] -> []
  | h::t -> h::[] 

let rec matchHeads x = match explode x with 
  | h::t -> if (getHeads (explode x)) = [] 
      then true 
      else if 
        (h::[]) = getHeads t
      then matchHeads(loseTail t)
      else false
