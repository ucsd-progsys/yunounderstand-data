...
foo [(fun x -> x+1); (fun y -> y-1)] 1;;
foo [] 1



let pipe fs = 
  let f a x = fun xs -> x (a xs) in  (*XXXXXXXXXXXXXXXXXXXXXXXXXXXXX
XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX*)
  let base = (fun b -> b) in (*XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
...
let bigMul l1 l2 = 
  ...
  let args = 
    ...
    let rec helper acc l1 ll2 = 
      match ll2 with
        | []   -> acc
        | h::t -> helper (List.append [(l1,h)] acc) t in
      helper [] l1 l2 in
  let (_, res) = List.fold_left f base args in
    res
