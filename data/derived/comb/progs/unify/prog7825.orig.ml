...
let rec exprToString e = match e with
  ...
  | Volume (j, k, l)    -> exprToString e ^ "*" ^ exprToString e "*" exprToString e
...
let buildVolume(j, k, l)           = Volume(j, k, l)
...
let rec eval (e,x,y) = match e with
  ...
  | Volume(j, k, l)     -> (eval(j, x, y) *. eval(k, x, y) *. eval(l, x, y))
...
let rec build (rand, depth) = if (depth = 0) then
    ...
    let random = rand(0,7) in
      match random with
        ...
        | _     -> buildSubstract(build(rand, depth-1), build(rand, depth-1))
