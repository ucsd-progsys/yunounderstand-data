...
let rec exprToString e = match e with
  ...
  | Thresh (th1, th2, th3, th4) -> "(" ^ (exprToString th1) ^ "<" ^ 
                                   ...
                                   ":" ^ (exprToString th4) ^ ")"
  | DivAdd (da1, da2, da3, da4) -> "((" ^ (exprToString da1) ^ "+" ^
                                   (exprToString da2) ^ ") / (" ^ (exprToString da3) ^
                                   "+" ^ (exprToString da4) ^ "))"
...
let rec eval (e,x,y) = 
  match e with 
    ...
    | DivAdd (da1, da2, da3, da4) ->
        if (eval(da1, x, y)) < (eval(da2, x, y)) then ((eval(da1,x,y)) /. (eval(da2, x, y)))
        else if (eval(da1, x, y)) > (eval(da2, x, y)) then (eval(da3, x, y)) < (eval(da4, x, y))
    | TriMult (tm1, tm2, tm3) -> (eval(tm1, x, y)) *. (eval(tm2, x, y)) *. (eval(tm3, x, y))
