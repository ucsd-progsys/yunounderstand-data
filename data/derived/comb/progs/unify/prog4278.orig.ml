...
let sampleExpr2=Nom(VarX,VarY,VarX);;
...
let rec build (rand, depth) = let res=(rand(0,69)mod 7) in match depth with
    ...
    |_ ->let nd=depth-1 in
          match res with
            ...
            |_->buildNom(build(rand,nd),build(rand,nd),build(rand,nd))
