...
let padZero l1 l2 = (List.append (clone 0 (List.length l2 - List.length l1)) l1, 
...
let bigAdd l1 l2 = 
  let add (l1, l2) = 
    let f a x = 
      let lr1 = List.rev l1 in let lr2 = List.rev l2 in
      let h1::t1 = lr1 in let h2::t2 = lr2 in
        if h1+h2 > 9 then x((1,[(h1+h2)-10])::a)
        else x((0,[h1+h2])::a)
...
let comb l1 l2 = List.combine l1 l2
let _= comb [1;0;5] [2;3;2]
let _ = List.rev(List.combine [(1,1);(0,0);(5,5)] [(2,2);(6,6);(7,7)])
