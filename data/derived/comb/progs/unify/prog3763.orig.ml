...
let rec assoc (d,k,l) = 
  match l with
    |[] -> d
    |h::t -> (match h with 
               |(k',d') -> if k' = k then d' else assoc(d,k,t) )
...
let _ = removeDuplicates [4;1;2;1;2;1;1;2;2;3;3;3;3;3;4];;

let _ = removeDuplicates ["a"; "b"; "a"; "cat"; "cake"; "cat"; "woah"; "woah"; "woah"; "cake"; "b"];;
...
let rec exprToString e = 
  match e with
    ...
    | Range (a,b,c) -> "range(" ^ exprToString a ^ "," exprToString b ^ "," ^ exprToString c ^ ")"
...
let buildMedian(e1,e2,e3)          = Median(e1,e2,e3)
...
let rec eval (e,x,y) = 
  match e with
    ...
    |Inverse a -> let result  = eval(a) in
          if result = 0. then
            0 
          else
            1 /. result
    |Max (a,b) -> let aResult = eval(a) in
        let bResult = eval(b) in
          if aResult > bResult then aResult
          else bResult
    |Range (a,b,c) -> let aResult = eval(a) in
        let bResult = eval(b) in
