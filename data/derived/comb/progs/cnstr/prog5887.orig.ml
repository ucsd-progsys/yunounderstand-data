...
let rec exprToString e = 
  match e with
    ...
    | SqXPlusY (e1,e2)      -> "("^(exprToString e1)^" * "^(exprToString e1)^") + ("^(exprToString e2)^"/2"
;;
...
let buildSqXPlusY(e1,e2)           = SqXPlusY(e1,e2)
...
let rec eval (e,x,y) = 
  match e with
    ...
    | SqXPlusY (e1,e2)      -> ((eval (e1,x,y)) *. (eval (e1,x,y)) +. (eval (e2,x,y) /. 2.0))
;;
...
let rec build (rand, depth) = 
  ...
  then (if (rand(0,1) = 0) 
    ...
    | _ -> buildSqXPlusY((build (rand, depth - 1)), (build (rand, depth -1)) )
;;
