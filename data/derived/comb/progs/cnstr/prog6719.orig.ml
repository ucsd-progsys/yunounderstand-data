...
let rec eval (e,x,y) = 
  match e with
    ...
    | Expn (a,b)	    -> let c = ((0.5)**eval(b,x,y)) in if c < 1.0 && c > -1.0 then c else (1.0/.c)
  ...
  if depth = 0 then 
    let b = rand(0,2) in 
      ...
      match b with 
        ...
        | 6 -> buildTripMult(build(rand,depth-1),build(rand,depth-1),build(rand,depth-1))
