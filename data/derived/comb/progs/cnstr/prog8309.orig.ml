...
let pi = 4.0 *. atan 1.0
let _ = Thresh(buildSine(buildCosine(VarX)), VarX, VarY, VarZ)
(*XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
...
let rec eval (e,x,y) = 
  match e with 
    | buildX -> x
    | buildY -> y
    | buildSine(a)-> cos(pi *. eval(a,x,y))
    | buildCosine(a) -> sin(pi *. eval(a,x,y))
    | buildAverage(e1,e2) -> (eval(e1,x,y) +. eval(e2,x,y)) /. 2.0
