...
let removeDuplicates l = 
  let rec helper (seen,rest) = 
    match rest with 
        [] -> seen
      | h::t -> 
          let seen' = if (List.mem h seen) then seen (*XXXXXXXXXXXXXXXX*)
            else h::seen in (*XXXXXXXXXXXXXX*)
          let rest' = t in (*XXXXXXXXXXXXXXX*)
            helper (seen',rest') 
  in
    List.rev (helper ([],l))
...
let rec wwhile (f,b) = 
  match (f b) with
    | (newB, false) -> newB (*XXXXXXXXXXXXXXXXXXXXXXXXX*)
    | (newB, true) -> wwhile(f, newB);;
...
let fixpoint (f,b) = wwhile ((fun x -> let newB = (f x) in (newB, newB!= x)),b)
...
let rec exprToString e =
  match e with 
    ...
    | Special1 (var11, var12, var13) -> "(sin(pi*" ^ exprToString var11 ^ ") * sin(pi*" ^ exprToString var11 ^ ") * cos(pi*" ^ exprToString var12 ^ ") * cos(pi*" ^ exprToString var12 ^ "))/(2*(" ^ exprToString var13 ^ ")"
    | Special2 (var14, var15) -> "(sin(pi*" ^ exprToString var14 ^ ") + cos(pi*" ^ exprToString var15 ^ "))/2"
...
let rec eval (e,x,y) = 
  match e with 
    ...
    | Special1 (var11, var12, var13) -> (sin(pi*.(eval(var11, x, y))) +. cos(pi*.(eval(var12, x, y))))/.2.0
    | Special2 (var14, var15) -> (sin(pi*.(eval(var14, x, y))) +. cos(pi*.(eval(var15, x, y))))/.2.0
...
let rec build (rand, depth) = 
  if (depth<1) then let base = rand (0, 2) in match base with 
      ...
      | 5 -> buildSpecial1(build(rand, depth-1), build(rand, depth-1))
      | _ -> if (recurse > 2) then buildCosine(build(rand, depth-1)) else buildSine(build(rand, depth-1))
