...
let rec assoc (d,k,l) = (*XXXXXXXXXXXXXXXXXXXXXXXX*)
  match l with
    |[] -> d
...
let removeDuplicates l = 
  let rec helper (seen,rest) = 
    match rest with 
      | [] -> seen
      | h::t -> 
          let seen' = (*XXXXXXXXXXXXXXXXXXXXXXXXXXX*)
            if List.mem h seen then
              seen
            else 
              h::seen
          in (*XXXXXXXXXXXXXXXXX*)
          let rest' = (*XXXXXXXXXXXXXXXXXXXXXXXXXXXX*)
            t
          in (*XXXXXXXXX*)
            helper (seen',rest')
...
XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
*)
let rec wwhile (f,b) = (*XXXXXXXXXXXXXXXXXXXXXXXXXX*)
  let helper = f b in
    match helper with
      | (x, y) -> if y = false
          then x
...
let fixpoint (f,b) = wwhile ((*XXXXXXXXXXXXXXXXXXXXXXXXXX*)
                       let g b = 
                         let helper = f b in
                           match helper with
                             | f-> if f = b
...
XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
*)
let rec exprToString e = (*XXXXXXXXXXXXXXXXXXXXXXXX*)
  match e with
      VarX           -> "x"
    | VarY           -> "y"
...
let rec eval (e,x,y) = (*XXXXXXXXXXXXXXXXXXXXXXXX*)
  match e with
      VarX           -> x
    | VarY           -> y
...
let rec build (rand, depth) = (*XXXXXXXXXXXXXXXXXXXXXXXXXXXX*)
  let rng = rand(0,9) in
    if depth >= 0 then 
      match rand (0,2) with
        | 0 -> buildX()
...
let g1 () = (*XXXXXXXXXXXXXXXXXXXXXXXXXXXXXX*)
  (4, 250, 500)
let g2 () = (*XXXXXXXXXXXXXXXXXXXXXXXXXXXXXX*)
  (3, 125, 250)
let g3 () = (*XXXXXXXXXXXXXXXXXXXXXXXXXXXXXX*)
  (5, 62, 125)

let c1 () = (*XXXXXXXXXXXXXXXXXXXXXXXXXXXX*)
  (6, 686, 968)
let c2 () = (*XXXXXXXXXXXXXXXXXXXXXXXXXXXXX*)
  (8, 43, 92)
let c3 () = (*XXXXXXXXXXXXXXXXXXXXXXXXXXXXX*)
  (7, 0, 127)
