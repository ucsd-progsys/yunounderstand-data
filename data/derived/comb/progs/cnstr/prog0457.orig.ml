...
let removeDuplicates l = 
  let rec helper (seen,rest) = 
    match rest with 
        [] -> seen
      | h::t -> 
          let seen' = (*XXXXXXXXXXXXXXXXXXXXXXXXX*) 
            if List.mem h seen then seen else h::seen in
          let rest' = (*XXXXXXXXXXXXXXXXXXXXXXXX*) 
            t in 
            helper (seen',rest') 
  in
    List.rev (helper ([],l))
...
let collatz n = match n with 1 -> 1 | _ when n mod 2 = 0 -> n/2 | _ -> 3* + 1;;
...
XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
*)
let rec exprToString e =  (*XXXXXXXXXXXXXXXXXXXXXXXXX*)
  match e with
    | VarX                    -> "x"
    | VarY                    -> "y"
    ...
    | Op2 (e1, e2, e3)    -> "(" ^ exprToString e1 ^ ">" ^ exprToString e2 ^
                             "?" ^ exprToString e3 ^ ":(" ^ exprToString e1 ^ "-" ^ exprToString e2 ^ "))"
...
let rec eval (e,x,y) = (*XXXXXXXXXXXXXXXXXXXXXXXX*)
  match e with
    ...
    | Op2 (e1, e2, e3, e4)    -> if eval(e1,x,y) > eval(e2,x,y) 
        then eval(e3,x,y) else eval(e1,x,y) -. eval(e2,x,y)
...
let rec build (rand, depth) = (*XXXXXXXXXXXXXXXXXXXXXXXXXXXXX*)
