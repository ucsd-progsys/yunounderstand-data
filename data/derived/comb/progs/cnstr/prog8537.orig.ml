...
let rec exprToString e = match e with 
  ...
  | Circ	(circ1,circ2,circ3)       -> "sqrt(|1-"^exprToString(circ1)^"^2|)"
  | Oscillate   m4		    -> "("^exprToString(m4)^"/((1-"^exprToString(m4)^")^2+"^exprToString(m4)^"^2))"
...
let rec eval (e,x,y) = match e with 
  ...
  | Circ	(circ1,circ2,circ3)       -> sqrt(abs_float (1.0-.eval(circ1,x,y)**2.0))
  | Oscillate   m4	    -> let x = eval(m4,x,y) in x/.sqrt(((1.0-.x)**2.0)+.(x**2.0))
...
let rec build (rand, depth) = if depth = 0 
  then match rand(0,2) with
    ...
    match rand(0,25) with
      | 0 -> if(depth<5) 
          then buildX()
          else let x = build(rand, depth-1) in buildCirc(x,build(rand, depth-1),x)
      | 1 -> if(depth<5) 
          then buildY()
          else let x = build(rand, depth-1) in buildCirc(x,build(rand, depth-1),x)
      ...
      | 6 | 12   -> buildThresh(build(rand, depth-1),build(rand, depth-1)
                               ,build(rand, depth-1),build(rand, depth-1))
      | 17| 18  | 16 |19 | 23 | 24  -> let x = build(rand, depth-1) in buildCirc(x,build(rand, depth-1),x)
      | 20| 21  |22   -> buildOscillate(build(rand, depth-1))
      | _ -> buildX();;
