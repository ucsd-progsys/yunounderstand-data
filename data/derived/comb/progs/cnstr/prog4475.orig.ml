...
let fixpoint (f,b) = let result = f b in
...
let rec build (rand,depth) = 
  if depth = 1 then 
    ...
    match rand(0,7) with
      ...
      | 4 -> buildHalve (build (rand,depth-1))
      | 5 -> buildWow (build (rand, depth-1), build (rand, depth-1), 
                       build (rand,depth-1))
