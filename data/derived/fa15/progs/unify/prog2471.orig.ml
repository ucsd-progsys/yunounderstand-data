...
let rec assoc (d,k,l) = if(l = []) then d else
    let h::t = l in
    let (x,y) = h in if(x = k) then y else assoc(d,k,t)
...
let _ = removeDuplicates[1;1;1;2;2;3;2;1;4];;
...
let rec exprToString e = 
  match e with
    ...
    | Square c        -> "("^exprToString c^")^2"
    | Poly(a,b,c,d)     -> "("^exprToString a^"*"^exprToString d^"^2+"
                           ^exprToString b^"*"^exprToString d^"+"
                           ^exprToString c^")"
...
let buildPoly(a,b,c,d)		   = Poly(a,b,c,d)
...
let rec eval (e,x,y) = 
  match e with
    ...
    | Poly(a,b,c,d)   -> (eval (a,x,y) *. eval (Square(d),x,y)) +.
                         (eval (b,x,y) *. eval (d,x,y)) +. eval (c,x,y)
...
let rec build (rand, depth) = 
  let num = rand(0,9) in
    match depth with
      | 0 -> if( (num mod 2) = 0 ) then buildX() else buildY()
      | n -> match num with
        | 0 -> build(rand,depth-1)
        | 1 -> buildSine(build(rand, depth-1))
        | 2 -> buildCosine(build(rand, depth-1))
        | 3 -> buildTimes(build(rand,depth-1),build(rand,depth-1))
        | 4 -> buildAverage(buildX(), build(rand,depth-1))
        | 5 -> build(rand,depth-1)
        | 6 -> buildThresh(buildX(), buildY(), buildSine(build(rand,depth-1)),
                           buildCosine(build(rand,depth-1)))
        | 7 -> buildSine(buildTimes(buildX(), build(rand, depth-1)))
        | 8 -> buildCosine(buildTimes(buildY(), build(rand, depth-1)))
        | 9 -> buildPoly(buildSine(build(rand, depth-1)),
                         buildCosine(build(rand, depth-1)),
                         buildAverage(buildX(),buildY()),
                         build(rand,depth-1))
