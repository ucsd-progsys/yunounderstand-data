...
let _ = removeDuplicates [0;0;1;2;3;4;4];;
let _ = removeDuplicates[[1;2;3];[2;3;4];[1;2;3]];;
(**)
...
let rec exprToString e = 
  let ex = exprToString in 
    match e with
      ...
      | Times(x,y) -> ex(x)^"*"^ex(y)
      | Thresh(w,x,y,z) -> "("^ex(w)^"<"^ex(x)^"?"^ex(y)^":"^ex(z)^")"
      | Power(x,y) -> ex(x)^"^"^ex(y)
      | Tower(x,y, z) -> "(1/("^ex(x)^"^"^ex(y)^"^"^ex(z)^"))"  

(*XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX*)

let sampleExpr1 = Thresh(VarX,VarY,VarX,(Times(Sine(VarX),Cosine(Average(VarX,VarY)))));;
let sampleExpr5 = Tower(VarX, VarY, VarY)
...
let buildTower(a, b, c)            = Tower(a, b, c)
...
let rec eval (e,x,y) = 
  match e with 
    ...
    | Power(a, b) -> if(x < 1.0 && x > -1.0 || y < 1.0 && y > -1.0) then 0.5 else eval(a, x, y) mod eval(b, x, y)
    | Tower(a, b, c) -> if(x < 1.0 && x > -1.0 || y < 1.0 && y > -1.0) then 0.5 else eval(a, x, y) +. eval(b, x, y) +. eval(c, x, y)
...
let rec build (rand, depth) = 
  match (rand(243, 987), depth) with 
      (x, 0) when x mod 2 = 0 -> buildY()
    | (x, 0) when x mod 2 = 1 -> buildX()
    | (x, n) when n > 0 && x mod 7 = 0 -> buildSine(build(rand, n - 1))
    | (x, n) when n > 0 && x mod 7 = 1 -> buildCosine(build(rand, n - 1))
    | (x, n) when n > 0 && x mod 7 = 2 -> buildAverage(build(rand, n - 1), build(rand, n - 1))
    | (x, n) when n > 0 && x mod 7 = 3 -> buildTimes(build(rand, n - 1), build(rand, n - 1))
    | (x, n) when n > 0 && x mod 7 = 4 -> buildPower(build(rand, n - 1), build(rand, n - 1))
    | (x, n) when n > 0 && x mod 7 = 5 -> buildTower(build(rand, n - 1), build(rand, n - 1), build(rand, n - 1))
    | (x, n)                           -> buildThresh(build(rand, n - 1), build(rand, n - 1), build(rand, n - 1), build(rand, n - 1)) 
