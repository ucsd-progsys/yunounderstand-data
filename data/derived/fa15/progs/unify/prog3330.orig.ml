...
let fixpoint (f,b) = wwhile((let g b = let t = f b in 
                                 if b = t then (b, false) 
                                 else (t, true) in g), b);;
...
let rec exprToString e = 
  match e with
    ...
    |AveThree(e1,e2,e3) -> "(("^exprToString e1^"+"^exprToString e2^"+"^exprToString e3^")/3)"
...
let sampleExpr2 = Neg(Thresh(VarX,VarY,VarX,(Times(Sine(VarX),Cosine(Average(VarX,VarY))))));;
let sampleExpr3 = Neg(AveThree(VarX,VarY,VarY));;

let _ = exprToString sampleExpr1 ;;
let _ = exprToString sampleExpr2;;
let _ = exprToString sampleExpr3;;
...
let buildAveThree(e1,e2,e3)        = AveThree(e1,e2,e3)
let buildNeg(e)                    = Neg(e)
...
let rec eval (e,x,y) = 
  match e with
    ...
    |AveThree(e1,e2,e3) -> (eval(e1,x,y) + eval(e2,x,y)) + eval(e3,x,y)) /. 3.0
...
let _ = eval (Neg(AddMul(VarX,VarY,VarY)),0.1,0.2);;
...
let rec build (rand, depth) = 
  if depth > 0 then 
    match rand(0,7) with
      ...
      |5 -> buildNeg(build(rand,depth-1))
      |_ -> buildAddMul(build(rand,depth-1),build(rand,depth-1),build(rand,depth-1))
  else
    match rand(0,2) with
      |0 -> buildX()
      |_ -> buildY()
