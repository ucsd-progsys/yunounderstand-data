...
let rec exprToString e = match e with 
  ...
  | Circ	(circ1,circ2)       -> "("^exprToString(circ1)^"^2+"^exprToString(circ2)^")"
...
let rec eval (e,x,y) = match e with 
  ...
  | Circ	(circ1,circ2)       -> exp eval(circ1,x,y) 2.0+ exp eval(circ2,x,y) 2.0
...
let rec build (rand, depth) = if depth = 0 
  then match rand(0,2) with
    ...
    match rand(0,23) with
      ...
      | 20| 21  |22   -> buildNatLog(build(rand, depth-1))
      | _ -> buildX();;
...
let _= doRandomGray (11,11,11);;
let _= doRandomColor (11,11241,14214124);;
