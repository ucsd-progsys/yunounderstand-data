...
let fixpoint (f,b) = let f' b = let b' = f b in (b',b <> b') in wwhile (f',b)
...
let rec exprToString e = match e with 
  ...
  | Thresh   (expr1,expr2,expr3,expr4) -> 
      ...
      ^ exprToString expr4 ^ ")" 
  | Golden expr                        ->
      "cos("^exprToString expr^"^2-"^exprToString expr^"-1)"
  | MeanPi (expr1,expr2,expr3) -> 
      "sin(("
      ^exprToString expr1^"+"
      ^exprToString expr2^"+"
      ^exprToString expr3^")/pi)";;

(*XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX*)

let sampleExpr1 = Thresh(VarX,VarY,VarX,(Times(Sine(VarX),Cosine(Average(VarX,VarY)))));;

let _ = exprToString (MeanPi(VarX,VarY,VarX))
...
let buildGolden(e)                 = Golden(e)
let buildMeanPi(e1,e2,e3)          = MeanPi(e1,e2,e3)
  ...
  then 
    let r = rand(1,7)  in 
      match r with 
        ...
        | 3 -> buildAverage(buildY(),build (rand, depth-1))
        | 4 -> buildTimes(buildX(),build (rand, depth-1))
        | 5 -> 
            buildThresh(build (rand, depth-1),build (rand, depth-1),
                        build (rand, depth-1),build (rand, depth-1))
        | 6 -> buildGolden(build (rand, depth-1))
        | _ -> buildMeanPi(buildX(),
                           buildY(),
                           build (rand, depth-1))
  else 
    buildX() ;;
