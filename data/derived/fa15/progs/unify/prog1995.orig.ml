...
let rec assoc (d,k,l) = match l with
  | [] -> d
  | h::t -> let (name, num) = h in
        if name = k then num else assoc(d, k, t);;
...
XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
*)
let rec wwhile (f,b) = let pair = f b in
  let (b', c') = pair in
...
let rec exprToString e = match e with
  ...
  | Average(h, k) -> "(("^exprToString h ^"+"^exprToString k^")/2)"
  | Times(x, y) -> exprToString x^"*"^exprToString y
  | Thresh(a, b, c, d) -> "("^exprToString a^"<"^exprToString b^"?"^exprToString c^":"^exprToString d^")"
  | Abs x -> "|"^exprToString x^"|"
  | Flip (a, b, c) -> "("^exprToString a^">"^exprToString b^"?"^exprToString c^":-1*"^exprToString c^")";;
...
let buildFlip(a, b, c)		   = Flip(a, b, c)
...
let rec eval (e,x,y) = match e with
  ...
  | Abs v -> if eval(v, x, y) < 0.0 then eval(v, x, y) *. -1.0 else eval(v, x, y)
  | Flip (a, b, c) -> if eval(a, x, y) > eval(b, x, y) then eval((c *. -1.0), x, y) else eval (c, x ,y);;
...
let rec build (rand, depth) = 
  let constraints = if depth = 0 then (0, 1)
    else (0, 6) in
    match rand constraints with
      | 0 -> buildX()
      | 1 -> buildY()
