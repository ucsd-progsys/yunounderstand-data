...
let padZero l1 l2 = 
  let n = (List.length l1) - (List.length l2) in
    if (n < 0) (*XXXXXXXXXXXXXXXXXXXXXXX*)
...
let _ = let (x1,x2) = (9,2) in (0, x1+x2::[])

let bigAdd l1 l2 = 
  let add (l1, l2) = 
    let f a x = let (x1,x2) = x in let (a1,a2) = a in 
        if (a1+x1+x2 >= 10) 
...
let rec mulByDigit i l = 
  ...
  then l
  else (mulByDigit (i-1) l) + (bigAdd l l);;
