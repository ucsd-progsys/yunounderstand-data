...
let fixpoint (f,b) = wwhile ((let helper x = 
                                let y = f x in
                                  if y = x then(y, false) 
                                  else (y, true)		
                              in helper), b)
...
let rec exprToString e = match e with
  ...
  | Sqrt e1 -> "sqrt(abs(" ^ (exprToString e1) ^ "))"
  | Average (e1, e2) -> "((" ^ (exprToString e1) 
                        ^ "+" ^ (exprToString e2) ^ ")/2)"
  | Times (e1, e2) -> (exprToString e1) ^ "*" ^ (exprToString e2)
  | ExtremeExp (e1, e2, e3) -> "((" ^ (exprToString e1) ^ "**" 
                               ^ (exprToString e2) ^ ")**" 
                               ^ (exprToString e3) ^ ")"
...
let buildExp(e1,e2,e3)             = ExtremeExp(e1,e2,e3)
...
let rec eval (e,x,y) = match e with
  ...
  | ExtremeExp (e1, e2, e3) -> ((eval (e1, x, y)**eval (e2, x, y))
                                **eval(e3, x, y))
  | Thresh (e1, e2, e3, e4) -> if eval (e1, x, y) < eval (e2, x, y)
      then eval (e3, x, y)
      else eval (e4, x, y)

let _ = eval (VarX, 1.0, 0.0);;
let _ = eval (Average(VarX,VarY), 5.0, 4.0);;
...
let rec build (rand, depth) = let selector = rand(0,9) in if depth = 0 
    then if (selector mod 2) = 0
      then buildX() 
      else buildY()
    else let x = rand(0,4) in 
        match x with
          |0 -> buildSine(build(rand, (depth-1))) 
          |1 -> buildCosine(build(rand, (depth - 1)))
          |2 -> buildSqrt(build(rand, depth - 1))
          |4 -> buildAverage(build(rand, (depth-1)),build(rand, (depth-1)))
          |5 -> buildTimes(build(rand, (depth-1)),build(rand, (depth-1)))
          |6 -> buildExp(build(rand, (depth-1)),build(rand, (depth-1)),build(rand, (depth-1)))
