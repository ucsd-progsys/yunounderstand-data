...
let rec digitsOfInt n = 
  if n<=0 then []
  else let x= n mod 10 and n=n/10 in digitsOfInt n@x::[];;
...
let myHelper n x=
  let n=digitsOfInt n in sumList n in
  increment x;;

let rec additivePersistence n = 
  if n < 10 then sumList x
  else let x=myHelper n [] in additivePersistence x;;


let rec otherHelper n c =
  let c = c+1 and n =digitsOfInt n in sumList n ;;
...
let helper n =
  let n=digitsOfInt n in sumList n;;

let rec digitalRoot n =  if n<10 then n
  else  let x = helper n in digitalRoot x;;
