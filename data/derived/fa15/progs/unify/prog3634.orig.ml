...
let rec exprToString e = match e with
  ...
  | Super(v,w)       -> "(" ^ exprToString v ^ "+" ^ exprToString w ^ ") *" ^ exprToString v 
...
let buildDivide(a,b)               = Divide(a,b)
let buildSuper(a,b)                = Super(a,b)
...
let rec eval (e,x,y) = let pi = 3.142 in 
    match e with 
      | VarX             -> x
      | VarY             -> y
      ...
      | Thresh(v,w,q,r)  -> if (eval (v,x,y) < eval (w,x,y)) then eval (q,x,y) 
          else eval (r,x,y)
...
let rec build (rand, depth) = match depth with 
  ...
  | depth -> let k = build (rand, depth-1) in
        match rand(0,7) with
          | 0 -> buildSine(k) 
          | 1 -> buildCosine(k)
          | 2 -> buildAverage(k, k)
          | 3 -> buildTimes(k, k)
          | 4 -> buildThresh(k, k, k, k)
          | 5 -> buildDivide(k,k)
          | 6 -> buildSuper(k,k)
