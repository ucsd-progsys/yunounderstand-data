...
let helper l1 l2 = 
  ...
  let (_, res) = List.fold_left fn (0, []) (List.rev l2) in 
    res

let _ = helper [2; 1] [3;2]

let helper l1 l2 =
  let fn b y = let (u, (v, w)) = b in ((u+1), (0, ((mulByDigit y l1) @ (clone 0 u))::w))  in
  let base = (0, (0, [])) in
  let (_, res) = List.fold_left fn base (List.rev 12) in 
    res

let helper2 curList lt cnt = match lt with
  | [] -> curList
  | h::t -> (
