...
let rec exprToString e = match e with 
  ...
  | Oscillate   m4		    -> "ln(|"^exprToString(m4)^"|)"
...
let rec eval (e,x,y) = match e with 
  ...
  | Circ	(circ1,circ2)       -> sqrt(abs_float (1.0-.eval(circ1,x,y)**2.0))
  | Oscillate   m4	    -> let x = eval(m4,x,y) in x/.sqrt(eval(m4,x,y)**2+.x**2)
...
let rec build (rand, depth) = if depth = 0 
  then match rand(0,2) with
    ...
    match rand(0,25) with
      | 0 -> if(depth<5) 
          then buildX()
          else buildCirc(build(rand, depth-1),build(rand, depth-1))
      | 1 -> if(depth<5) 
          then buildY()
          else buildCirc(build(rand, depth-1),build(rand, depth-1))
      ...
      | 6 | 12  | 16 | 23 | 25-> buildThresh(build(rand, depth-1),build(rand, depth-1)
...
let _= doRandomGray (11,5,7);;
let _= doRandomColor (11,100,400);;
