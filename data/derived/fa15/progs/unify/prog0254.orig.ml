...
let rec exprToString e = match e with 
  ...
  | Funny1(n)           -> "sqrt(abs_float"^exprToString n^")/1000)"
...
let rec eval (e,x,y) = match e with
  ...
  | Funny(k, l, m)       -> ((eval(k,x,y) +. eval(l,x,y) +. eval(m,x,y))/.10)
...
let rec build (rand, depth) = match rand(0,8) with
  ...
  | 6 -> if (depth = 0) then (if (rand(0,2) = 0) then buildX() else buildY()) else buildFunny(build(rand,depth-1), build(rand,depth-1), build(rand,depth-1)) 
