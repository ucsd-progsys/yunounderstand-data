...
let fixpointHelper f b = fun b-> let c = f b in (c, (if c = b then false else true))
;; 
...
let rec eval (e,x,y) = match e with
  ...
  | Thresh(e1,e2,e1less,e2less)-> if eval(e1,x,y) < eval(e2,x,y)
      then eval(e1less,x,y) else eval(e2less,x,y)
;;
...
let c1 () = failwith (6,3,5)
let c2 () = failwith (4,2,7) 
