...
let rec exprToString e = match e with
  ...
  | Uncreative (e1, e2, e3)-> "("^exprToString(e1)^"/2*"^exprToString(e2)^"/3*"^exprToString(e3)^"/4)"
...
let sampleExpr5 = Uncreative(Creative(Thresh(VarX,VarY,VarX,(Times(Sine(VarX),Cosine(Average(VarX,VarY)))))),VarX,VarY);;
let _ = exprToString sampleExpr5;;
...
let rec eval (e,x,y) = match e with
  ...
  | Uncreative (e1, e2, e3) -> eval(e1,x,y)/2* eval(e2,x,y)/3*eval(e3,x,y)/4
...
let rec build (rand, depth) = 
  if depth = 0 then let x = rand(1,2) in 
      if x = 1 then VarX else VarY
  else let x = rand(1,7) in
      match x with 
          1 -> build(rand, depth-1)
        |2-> build(rand, depth-1)
        |3->Sine(build(rand, depth-1))
        |4->Cosine(build(rand, depth-1))
        |5->Average(build(rand, depth-1),build(rand, depth-1))
        |6->Times(build(rand, depth-1),build(rand, depth-1))
