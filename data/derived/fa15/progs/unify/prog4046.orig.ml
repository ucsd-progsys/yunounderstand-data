...
let rec assoc (d,k,l) = match l with
  | [] -> d
  | h :: t -> let (f, s) = h in 
        if k = f then s else assoc (d, k, t)
;;
...
let fixpoint (f,b) = let f' b' = (f b', (f b') != b') in
    wwhile(f', b)
;;
...
let rec exprToString e = match e with
  ...
  | ThreeAverage(e1, e2, e3) -> "((" ^ exprToString(e1) ^ "+" ^ exprToString(e2) ^ "+" ^ exprToString(e3) ^ ")/3)"
...
let sampleExpr1 = Thresh(VarX,VarY,VarX,(Times(Cube(Sine(VarX)),Cosine(ThreeAverage(VarX,VarY,VarX)))));;
...
let buildThreeAverage(e1, e2, e3)  = ThreeAverage(e1, e2, e3)
...
let rec eval (e,x,y) = match e with
  ...
  | ThreeAverage(e1, e2, e3) -> (eval(e1, x, y) +. eval(e2, x, y) +. eval(e3, x, y)) /. 3.0
...
let rec build (rand, depth) = 
  if depth = 0 then let j = rand (1, 3) in if j = 1 then buildX() else buildY()
  else let j = rand(1, 6) in
      match j with 
