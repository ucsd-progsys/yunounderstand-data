...
let rec assoc (d,k,l) = 
  match l with
    | [] -> -1
    | h::l' -> let (x,y) = h in
          if x = k then y else assoc(d,k,l')
(*XXXXXXXX
...
let fixpoint (f,b) = wwhile ((let checkB b' = 
                                let c = f(b') in 
                                  (c,b'!=c) in checkB), b)
...
let rec exprToString e = 
  match e with
    | VarX              -> "x" (*XXXXXXXXXXXXXXXXXXXXXXXXXXXXX*)
    | VarY              -> "y" (*XXXXXXXXXXXXXXXXXXXXXXXXXXXXX*)
    | Sine b            -> "sin(pi*"^exprToString(b)^")" (*XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX*)
    | Cosine b          -> "cos(pi*"^exprToString(b)^")" (*XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX*)
    | Average (a,b)     -> "(("^exprToString(a)^"+"^exprToString(b)^")/2)" (*XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX*)
    | Times   (a,b)     -> exprToString(a)^"*"^exprToString(b) (*XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX*)
    | Thresh  (a,b,c,d) -> "("^exprToString(a)^"<"^exprToString(b)^"?"^exprToString(c)^":"^exprToString(d)^")"(*XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX*)
    (*XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX*)
    | Logx (a)	    -> "(log("^exprToString(a)^"))"
    | TripMult (a,b,c)  -> "("^exprToString(a)^"*"^exprToString(b)^"*"^exprToString(c)^")"
    | _                 -> ""
...
let rec eval (e,x,y) = 
  match e with
    | VarX              -> x (*XXXXXXXXXXXXXXXXXXXXXXXXXXXXX*)
    | VarY              -> y (*XXXXXXXXXXXXXXXXXXXXXXXXXXXXX*)
    | Sine b            -> (sin(pi*.eval(b,x,y))) (*XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX*)
    | Cosine b          -> (cos(pi*.eval(b,x,y))) (*XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX*)
    | Average (a,b)     -> (((eval(a,x,y))+.(eval(b,x,y)))/.2.0) (*XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX*)
    | Times   (a,b)     -> (eval(a,x,y))*.(eval(b,x,y)) (*XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX*)
    | Thresh  (a,b,c,d) -> if (eval(a,x,y))<(eval(b,x,y)) then (eval(c,x,y)) else (eval(d,x,y))(*XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX*)
  ...
  if depth = 0 then 
    let b = rand(0,2) in 
      ...
      match b with 
        ...
        | 5 -> buildExpn(build(rand,depth-1),build(rand,depth-1))
...
let _ = doRandomColor(12,6,55) ;;
let _ = doRandomColor(7,25,200) ;;
