...
let rec exprToString e = 
  match e with
    ...
    | Average(s, t)      ->  "(("^ exprToString s^"+"^exprToString t^")/2)" 
    | Times(s, t)        -> exprToString s^"*"^exprToString t 
    | Thresh(s, t, u, v) -> "("^exprToString s^"<"^exprToString t^"?"^exprToString u^":"^exprToString v^")"
    | Square s -> "(" ^ exprToString s ^ ")^2"
    | Volume (s, t, u) -> "Vol(H: " ^ exprToString s ^ ", W: " ^ exprToString t ^ ", L: " ^ exprToString u ^ ")"
...
let buildVolume(h,w,l)             = Volume(h,w,l)
...
let rec eval (e,x,y) = match e with
  ...
  | Volume (vol_1,vol_2,vol_3) -> eval (vol_1,x,y) *. eval (vol_2,x,y) *. eval (vol_3,x,y)
...
let rec build (rand, depth) = 
  if depth == 0 then
    let a = rand (0, 1) in 
      if a == 0 then buildX() else buildY()
  else 
    let a = rand (0, 4) in 
      match a with
