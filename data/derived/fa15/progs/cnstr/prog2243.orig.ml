...
let _ = removeDuplicates [1;2;3;4;2;2;5];;
...
let rec exprToString e = 
  match e with 
    ...
    | Trip(x,y,z)      -> "(("^ exprToString x ^ "/30.0)+" ^ exprToString y ^
                          "/" ^ exprToString z ^ ")"
    | Greater(x,y)     -> "(" ^ exprToString x ^ ">" ^ exprToString y ^ 
                          "?" ^ exprToString x ^ ":" ^ exprToString y ^ ")"

let sampleExpr1 = Thresh(VarX,VarY,VarX,(Times(Sine(VarX),Cosine(Average(VarX,VarY)))));;
let sampleTrip = Greater(VarX, VarY)
let _ = exprToString sampleTrip
...
let buildTrip(e1, e2, e3)          = Trip(e1, e2, e3)
let buildGreater(e1, e2)           = Greater(e1, e2)
...
let rec eval (e,x,y) = 
  match e with
    ...
    | Thresh(t,r,s,q) -> 
        if ((eval(t,x,y)) < (eval(r,x,y))) then eval(s,x,y) 
        else eval(q,x,y)
    | Trip(t,r,s)     -> (sin(pi*.eval(r,x,y)) *. sin(pi*.eval(s,x,y))) 
                         *. sin(pi*.eval(t,x,y))
    | Greater(t, s)   -> 
        if eval(t,x,y) > eval(s,x,y) then eval(t,x,y)
        else eval(s,x,y)
...
let rec build (rand, depth) = 
  let r = rand(0, 11) in
    match depth with
      | 0 -> 
          if r mod 2 = 0 then buildX()
          else buildY()
      | d -> match r with
        | sel when sel <= 2 -> (buildSine (build (rand, d-1)))
        | 3 -> buildAverage((build (rand, d-1)), (build (rand, d-1)))
        | 4 -> buildTimes((build (rand, d-1)), (build (rand, d-1)))
        | 5 -> buildThresh((build (rand, d-1)), (build (rand, d-1)),
                           (build (rand, d-1)), (build (rand, d-1)))
        | 6 -> buildTrip((build (rand, d-1)), (build (rand, d-1)),
                         (build (rand, d-1)))
        | 7 -> buildGreater((build (rand, d-1)), (build (rand, d-1)))
        | sel when sel >= 8 -> (buildCosine (build (rand,d-1)))
;;
