...
let rec exprToString e = match e with
  ...
  | TimesThree e -> (exprToString e) ^  "*" ^ (exprToString f) ^ "*" ^ (exprToString f)
...
let buildTimesThree(e1,e2)         = TimesThree(e1,e2,e3)
...
let rec eval (e,x,y) = match e with
  |VarX -> x
  |VarY -> y
  |Sine e -> sin (3.142 *. eval (e,x,y))
  |Cosine e -> cos (3.142 *. eval (e,x,y))
  |TimesThree (e,f,g) -> ((eval (e,x,y)) *. (eval (f,x,y)) *. (eval (f,x,y)))
  |Average (e,f) -> (((eval (e,x,y))+.(eval (f,x,y)))/.2.0)
  |Times (e,f) -> ((eval (e,x,y)) *. (eval (f,x,y)))
  |Thresh (e,f,g,h) -> match ((eval (e,x,y)) < (eval (f,x,y))) with
    |true -> (eval (g,x,y))
    |false -> (eval (h,x,y));;
...
let rec build (rand, depth) = match rand(1,8) with
  |1 -> buildX()
  |2 -> buildY()
  ...
  |8 -> buildTimesThree (build (rand,depth-1), build (rand,depth-1) , build (rand,depth-1))
;;
